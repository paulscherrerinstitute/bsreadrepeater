...
#else
    nbytes = LZ4_decompress_safe((const char *)in + 4, (char *)tmp_buf, nbytes_from_header, size * elem_size);
    CHECK_ERR_FREE_LZ(nbytes, tmp_buf);
...
size_t bshuf_compress_lz4_bound(const size_t size, const size_t elem_size, size_t block_size) {
    ...
    if (leftover)
        bound += LZ4_compressBound(leftover * elem_size) + 4;
...
int64_t bshuf_compress_lz4(const void *in, void *out, const size_t size, const size_t elem_size, size_t block_size) {
    return bshuf_blocked_wrap_fun(&bshuf_compress_lz4_block, in, out, size, elem_size, block_size);
}

int64_t bshuf_decompress_lz4(const void *in, void *out, const size_t size, const size_t elem_size, size_t block_size) {
    return bshuf_blocked_wrap_fun(&bshuf_decompress_lz4_block, in, out, size, elem_size, block_size);
}
